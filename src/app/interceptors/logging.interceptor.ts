    ...
    tap({
      next: (event: HttpEvent<unknown>): void => {
        if (event instanceof HttpResponse) {
          console.log(
            `Метод: ${ cloneReq.method } Статус: ${ event.status } Запрос ${ cloneReq.url } выполнился за ${ duration }ms`,
          );
        }
      },
      ...
      error: (error: HttpErrorResponse): void => {
        ...
        console.error(
          `[HTTP ERROR] Метод: ${ cloneReq.method } URL: ${ cloneReq.url } ${ statusInfo } Время ${ duration }ms`,
        );
      },
    }),
  );
